...
//       console.log(data);
      ...
      itemHtml = itemHtml.replace(/{{vendorHandle}}/g, data.vendor.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-$/, '').replace(/^-/, ''));
		itemHtml = itemHtml.replace(/{{data_with_collection}}/g, Utils.buildProductItemUrl(data));
    ...
    function min_qty_indicator(product){
        ...
        data_tagArray.forEach(function(v,i){
          if(v.indexOf("min-qty:")> -1){
            ...
            html = ` <p>Minimum of <b>${qtyTagArray[1]} qty</b> <span class="popup_minq"><a href="#one" class="fancybox"><svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
...
//       console.log(data);
		...
		if (boostPFSThemeConfig.custom.hasOwnProperty('sold_out_badge_color_scheme') && soldOut) {
          ...
          soldOutLabel = boostPFSTemplate.soldOutLabelHtml.replace(/{{itemUrl}}/g, '/products/'+data.handle );
		}
		return soldOutLabel;
	}
...
//       render function end
      ...
      // variant prod click
      var variantProds = document.querySelector('.card-information a');
      console.log(variantProds);
       console.log( document.querySelector('.card-information a').getAttribute('data-with-collection').split('/products/')[1]);
      document.querySelector('.card-information a').onclick = function(event){
        ...
        window._swat.initializeActionButtons('.qv-product-options_html', '.swym-button');
      	...
      	jQ('.swatch-element').on("click", function(el) {
            ...
            jQ('.boost-pfs-modal-content product-form input[name="id"]').on('change',function(){
              	jQ(this).closest('.boost-pfs-modal-content').find('.swym-add-to-wishlist-view-product').attr('data-variant-id',jQ('.boost-pfs-modal-content product-form input[name="id"]').val());
            });
        });
      // swym 
