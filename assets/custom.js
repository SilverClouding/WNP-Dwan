...
//crosssell button click
//   var pairsButton = function() {}
  $(document).on('click','.pairs-with-treasure', function(e) {
    e.preventDefault();
    $(this).parents('.min-cart-item').children('.crosssell-group').toggleClass("hide");     
  });
   $(document).on('click','.pairs-with-treasure', function(e) {
    e.preventDefault();
    $(this).parents('.col-notimage').siblings('.crosssell-group').toggleClass("hide");     
  });
  
  $(document).on('click','.crosssell-group-close', function(e) {
     e.preventDefault();
    $(this).parents('.crosssell-group').toggleClass("hide"); 
  });
  ...
  //update cart item while remove cart item
  $(document).on('click','.btn-remove', function(e) {
    e.preventDefault();
    console.log("remove item");
    var shopifyAjaxChangeURL = theme.routes.cart_change_url+ '.js';
    var ItemId = $(this).attr("ItemId"); 
    var ItemLineNumber = $(this).attr("line"); 
    
    $.ajax({
      type: 'post',
      url: shopifyAjaxChangeURL,
      dataType: 'json',
      data: {quantity: 0, id: ItemId,sections: "cart-items"},
...
//     $.post(shopifyAjaxChangeURL, { quantity: 0, id: ItemId, sections: "cart-items" }, function (data) {
//       var sectionHtmlData =  $.parseJSON(data);
//       var SectionHtml = sectionHtmlData.sections['cart-items'] ;
//       $("#mini-cart").html(SectionHtml);
//       $('#mini-cart').addClass("show-minibag");
//       $('#mini-cart').removeClass("hide-minibag");
...
//     });
  ...
  //Slideout close while click on cross
  $(document).on('click','.close-bag, .minibag-mask', function() {
    console.log("leave");
    $('#mini-cart').removeClass("show-minibag");
    $('#mini-cart').addClass("hide-minibag");
