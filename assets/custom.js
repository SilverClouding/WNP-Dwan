...
//   const avtiveSlide = document.querySelector(`.brand_logos  .swiper-slide-next`); 
  ...
  avtiveSlide.style.marginLeft  = (55 / this.activeIndex)*this.activeIndex+this.activeIndex*1.2+"px";
...
jQuery(function($){
  ...
  $('[id^=qv_]').filter((idx,element)=>{
    ...
    console.log(element.id.match(/qv_\d\d\d\d\d\d\d\d\d\d\d\d\d\d\d/));
    return element.id.match(/qv_\d\d\d\d\d\d\d\d\d\d\d\d\d\d\d/) && ![0,15].includes(indexInParent);
  }).remove();
